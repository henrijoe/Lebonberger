...
import mtn from '../../images/logo/MTN.png';
import moov from '../../images/logo/moov.png';
import wave from '../../images/logo/wave.jpg';
import { FaUser, FaPhone, FaChurch, FaMoneyBillAlt } from 'react-icons/fa';
import { MdOutlineSupportAgent } from 'react-icons/md';
import { userSchema, UserState } from "../../schemas/utilisateurSchemas";
import { useDispatch } from "react-redux";
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm } from "react-hook-form";
import { setUserData } from '../../store/utilisateurSlice';
...
const SignUp: React.FC = () => {
  ...
  return (
    <>
      <Breadcrumb pageName="Compte le bon berger" />
        ...
        <div className="flex flex-wrap">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              <Link className="mb-5.5 inline-block" to="/">
                <img
                  className="rounded-full w-20 h-20 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-40 lg:h-40"
                  src={Logo}
                  alt="Logo"
                />
              </Link>
              <p className="2xl:px-20 text-black dark:text-white sm:text-title-xl2">
                Veuillez créer votre compte pour profiter de nos formations.
              </p>
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              {/* <span className="mb-1.5 block font-medium">Commencer</span> */}
              <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
                Création de compte Lebonberger
              </h2>

              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-4">
                  <label className="block font-medium text-black dark:text-white">
                    <FaUser className="inline mr-2 text-primary" /> Nom
                  </label>
                  <input
                  ...
                  {errors.nomUtilisateur && (
                    <p className="text-red-500">{errors.nomUtilisateur.message}</p>
                  )}
                </div>

                <div className="mb-4">
                  <label className="block font-medium text-black dark:text-white">
                    <FaUser className="inline mr-2 text-primary" /> Prénoms
                  </label>
                  <input
                  ...
                  {errors.prenomUtilisateur && (
                    <p className="text-red-500">{errors.prenomUtilisateur.message}</p>
                  )}
                </div>

                <div className="mb-4">
                  <label className="block font-medium text-black dark:text-white">
                    <FaPhone className="inline mr-2 text-primary" /> Numéro de téléphone
                  </label>
                  <input
                  ...
                  {errors.telephoneUtilisateur && (
                    <p className="text-red-500">{errors.telephoneUtilisateur.message}</p>
                  )}
                </div>

                <div className="mb-4">
                  <label className="block font-medium text-black dark:text-white">
                    <FaChurch className="inline mr-2 text-primary" /> Église locale
                  </label>
                  <input
                    {...register("eglise")}
                    type="text"
                    placeholder="Entrez le nom de votre église locale"
                    className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10"
                  />
                  {errors.eglise && (
                    <p className="text-red-500">{errors.eglise.message}</p>
                  )}
                </div>

                <div className="mb-4">
                  <label className="block font-medium text-black dark:text-white">
                    <MdOutlineSupportAgent className="inline mr-2 text-primary" /> Statut
                  </label>
                  <select
                    {...register("statut")}
                    className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10"
                  >
                    ...
                    <option value="Pasteur Membre">Pasteur Membre</option>
              ...
              <div className="mt-10">
                <h3 className="text-lg font-bold text-black dark:text-white">
                  <FaMoneyBillAlt className="inline mr-2 text-green-500" /> Pour tous ceux qu'ils veulent soutenir l'oeuvre
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Faites vos dons via les moyens de paiement suivants :
                </p>
                ...
                <ul className="mt-4 space-y-4 text-sm text-gray-700 dark:text-gray-300">
                  <li className="flex items-center space-x-4">
                    <img
                      ...
                      alt="Orange Money"
                    />
                    <span className='font-bold text-black dark:text-white text-lg'>Orange Money : 0789854100</span>
                  </li>
                  <li className="flex items-center space-x-4">
                    <img
                      className="w-10 h-10"
                      ...
                      alt="MTN Money"
                    />
                    <span className='font-bold text-black dark:text-white text-lg'>MTN Money : 0589854100</span>
                  </li>
                  <li className="flex items-center space-x-4">
                    <img
                      className="w-10 h-10"
                      ...
                      alt="Moov Money"
                    />
                    <span className='font-bold text-black dark:text-white text-lg'>Moov Money : 0103205541</span>
                  </li>
                  <li className="flex items-center space-x-4">
                    <img
                      className="w-10 h-10"
                      ...
                      alt="Wave"
                    />
                    <span className='font-bold text-black dark:text-white text-lg'>Wave : 0574313410</span>
                  </li>
                  <li className="flex items-center space-x-4">

                    <span className='font-bold text-black dark:text-white text-lg'>NB:</span>
                    <span className='font-bold text-black dark:text-white text-lg'>Pour tout autres paiements veuillez conctatez : +225 0789815111</span>
                  </li>
                </ul>
              </div>
            </div>
